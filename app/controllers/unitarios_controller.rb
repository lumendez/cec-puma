class UnitariosController < ApplicationController
  ...
  def imprimir_credenciales_media
    @unitarios = Unitario.where(documentos_validados: true).includes(:grupos_unitario).where(:grupos_unitarios => { nombre: "Curso propedéutico para el examen de admisión al Nivel Medio Superior" } ).all
    @multiplo = 4
    respond_to do |format|
      format.html
      format.pdf do
  ...
  def imprimir_credenciales_superior
    @unitarios = Unitario.where(documentos_validados: true).includes(:grupos_unitario).where(:grupos_unitarios => { nombre: "Curso de preparación para el examen de admisión al Nivel Superior" } ).all
    @multiplo = 4
    respond_to do |format|
      format.html
      format.pdf do
